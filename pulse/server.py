...
def generate_quark(message: str, history: list[dict[str, str]]) -> str:
    ...
    def _decode() -> tuple[list[int], bool]:
        """Sampled decode (temp 0.8, top-p 0.9) with a repetition penalty.
        ...
        for _ in range(140):
            ...
            sprobs, sidx = torch.sort(probs, descending=True, dim=-1)
            cum = torch.cumsum(sprobs, dim=-1)
            sprobs = torch.where(cum - sprobs > 0.9, torch.zeros_like(sprobs), sprobs)
            sprobs = sprobs / sprobs.sum(dim=-1, keepdim=True)
            best = int(sidx[0, int(torch.multinomial(sprobs[0], 1).item())])
